...
ggplot(data = activity_data_cc_day_int, aes(x = interval, y = day_int_avg, color = weekday)) + 
        geom_line() + stat_smooth(se = FALSE, color = "black", size = 0.5) + facet_grid(facets = weekday ~ .) + 
        scale_x_continuous(labels = x_interval_labels, breaks = x_interval) + 
...
```r
activity_data_imp$week_part <- ifelse(weekdays(activity_data_imp$date) %in% c("Saturday", "Sunday"), "Weekend", "Weekday")
